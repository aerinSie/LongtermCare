...
public class BalanceJDBCDAO implements BalanceDAO_interface {
	...
	private static final String INSERT_STMT_BONUS = "INSERT INTO BALANCE (TOPUP_NO, MEM_NO,TOPUP_VALUE,TOPUP_WAY) VALUES "
			+ "(to_char('TPP'||to_char(sysdate,'yymm')||LPAD(to_char(SEQ_EMPLOYEE.NEXTVAL),3,'0')),?,100,'SYSTEM')";

	// 設定：不刪
	// 設定：不改
	// 設定：可查
	private static final String GET_ONE_STMT = "SELECT TOPUP_NO, MEM_NO,TOPUP_VALUE,STATUS,TOPUP_WAY,"
			+ "to_char(TOPUP_TIME,'yyyy-mm-dd-hh:mm:ss') TOPUP_TIME FROM BALANCE WHERE MEM_NO=?";

	private static final String GET_ONE_TOPUPNO_STMT = "SELECT TOPUP_NO, MEM_NO,TOPUP_VALUE,STATUS,TOPUP_WAY,"
			+ "to_char(TOPUP_TIME,'yyyy-mm-dd-hh:mm:ss') TOPUP_TIME FROM BALANCE WHERE TOPUP_NO=?";

	private static final String GET_SOMEONE_STMT = "SELECT TOPUP_NO, MEM_NO,TOPUP_VALUE,STATUS,TOPUP_WAY,"
			+ "to_char(TOPUP_TIME,'yyyy-mm-dd-hh:mm:ss') TOPUP_TIME FROM BALANCE WHERE MEM_NO=?";

	// ---------------後端-------------------
	// 設定：不增
	// 設定：不刪 （下行為練習用的）
	private static final String DELETE = "DELETE FROM BALANCE WHERE TOPUP_NO = ?";

	// 設定：不改 （下行為練習用的）
	private static final String UPDATE = "UPDATE BALANCE set STATUS=? WHERE TOPUP_NO =?";

	// 設定：可查
	private static final String GET_ALL_STMT = "SELECT TOPUP_NO, MEM_NO,TOPUP_VALUE,STATUS,TOPUP_WAY,to_char"
	...
	public void insert_Bonus(BalanceVO balanceVO) {
		...
		try {
			...
			pstmt = con.prepareStatement(
					"update member  set point = (select point from member where mem_no = ?) where mem_no = ? ");
	...
	public static void main(String[] args) {
		...
		BalanceJDBCDAO dao = new BalanceJDBCDAO();
		//
		// // 新增
		 ...
		 * to_char(TOPUP_TIME,'yyyy-mm-dd-hh:mm:ss') TOPUP_TIME FROM BALANCE WHERE
		 * TOPUP_NO=?";
		 */
		...
		// "SELECT TOPUP_NO,
		// MEM_NO,TOPUP_VALUE,STATUS,TOPUP_WAY,to_char(TOPUP_TIME,'yyyy-mm-dd-hh:mm:ss')
		// TOPUP_TIME FROM BALANCE ORDER BY TOPUP_NO";
		// 查詢
